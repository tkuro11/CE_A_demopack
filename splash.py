...
def run_water_distortion():
    ...
    # 縞模様ベースのRGB配列
    base_r = (np.sin(xv) * 127 + 128).astype(np.uint8)
    base_g = (np.cos(yv) * 127 + 128).astype(np.uint8)
    ...
    while running:
        ...
        # 各行(Y座標)ごとに、X座標をどれだけズラすかをサイン波で計算
        # 行ごとに異なる位相の波を与えることで、うねるような水面を作る
        shift_x = (np.sin(y_indices[:, None] * 0.1 + t) * 15).astype(np.int32)
        
        # 各列(X座標)ごとも縦にズラす
        shift_y = (np.cos(x_indices[None, :] * 0.1 + t * 1.2) * 15).astype(np.int32)
        ...
        # 高級なNumPyインデキシングで、ピクセルを波形に沿って再配置（歪み発生）
